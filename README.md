...
By default no type generated by `Typely` accepts being created with a null reference. Default values for the value type are allowed. If you want the type to be set to a null value, declare it nullable (`MyType?`) or make the type a reference type with `AsClass()`.
