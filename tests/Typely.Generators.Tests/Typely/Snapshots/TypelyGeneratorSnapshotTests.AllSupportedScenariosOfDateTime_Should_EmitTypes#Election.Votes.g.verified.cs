...
namespace Election
{
    [TypeConverter(typeof(TypelyTypeConverter<DateTime, Votes>))]
    [JsonConverter(typeof(TypelyJsonConverter<DateTime, Votes>))]
    public readonly partial struct Votes : ITypelyValue<DateTime, Votes>, IEquatable<Votes>, IComparable<Votes>, IComparable
    {
        public DateTime Value { get; }

        private Votes(DateTime value, bool byPassValidation)
        {
            Value = value;
        }
        ...
        {
            TypelyValue.ValidateAndThrow<DateTime, Votes>(value);
            Value = value;
        }

        public static ValidationError? Validate(DateTime value)
        {
            if (value == default)
            {
            ...
            if (value <= new DateTime(2022, 1, 1))
            {
                return ValidationErrorFactory.Create(value, "GreaterThan", LocalizedMessages.CustomMessage, "Presidency vote",
                    new Dictionary<string, object?>
                    {
                        { "ComparisonValue", new DateTime(2022, 1, 1) },
                    });
            }
        ...
        public static Votes From(DateTime value) => new(value);

        public static bool TryFrom(DateTime value, out Votes typelyType, out ValidationError? validationError)
        {
        ...
        {
            if(DateTime.TryParse(value, out var underlyingValue))
            {
        ...
        public static explicit operator DateTime(Votes value) => value.Value;
    }
}