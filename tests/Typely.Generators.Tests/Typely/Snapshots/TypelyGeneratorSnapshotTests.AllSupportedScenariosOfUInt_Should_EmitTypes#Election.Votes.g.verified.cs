...
namespace Election
{
    [TypeConverter(typeof(TypelyTypeConverter<uint, Votes>))]
    [JsonConverter(typeof(TypelyJsonConverter<uint, Votes>))]
    public readonly partial struct Votes : ITypelyValue<uint, Votes>, IEquatable<Votes>, IComparable<Votes>, IComparable
    {
        public uint Value { get; }

        private Votes(uint value, bool byPassValidation)
        {
            Value = value;
        }

        public Votes(uint value)
        {
            TypelyValue.ValidateAndThrow<uint, Votes>(value);
            Value = value;
        }

        public static ValidationError? Validate(uint value)
        {
            if (value == default)
            {
        ...
        public static Votes From(uint value) => new(value);

        public static bool TryFrom(uint value, out Votes typelyType, out ValidationError? validationError)
        {
        ...
        {
            if(uint.TryParse(value, out var underlyingValue))
            {
        ...
        public static explicit operator uint(Votes value) => value.Value;
    }
}