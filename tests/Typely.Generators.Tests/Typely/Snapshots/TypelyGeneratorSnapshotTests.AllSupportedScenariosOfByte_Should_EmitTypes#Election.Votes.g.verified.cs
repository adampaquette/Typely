...
namespace Election
{
    [TypeConverter(typeof(TypelyTypeConverter<byte, Votes>))]
    [JsonConverter(typeof(TypelyJsonConverter<byte, Votes>))]
    public readonly partial struct Votes : ITypelyValue<byte, Votes>, IEquatable<Votes>, IComparable<Votes>, IComparable
    {
        public byte Value { get; }

        private Votes(byte value, bool byPassValidation)
        {
            Value = value;
        }

        public Votes(byte value)
        {
            TypelyValue.ValidateAndThrow<byte, Votes>(value);
            Value = value;
        }

        public static ValidationError? Validate(byte value)
        {
            if (value == default)
            {
        ...
        public static Votes From(byte value) => new(value);

        public static bool TryFrom(byte value, out Votes typelyType, out ValidationError? validationError)
        {
        ...
        {
            if(byte.TryParse(value, out var underlyingValue))
            {
        ...
        public static explicit operator byte(Votes value) => value.Value;
    }
}